...
@include('activity.sidebar')
    ...
    <div class="flex flex-col">
        <div> 
        @include('activity.overnightGallery')
        </div>
            ...
            @csrf
                <div class="flex flex-wrap -mx-3 mb-3">
                    <div class="w-full md:w-full px-3 mb-2 mt-2">
                        ...
                        <input type="hidden" name="commentType" value="overnightCMT">
                    </div>
      ...
      <div>
        @foreach($overnights as $overnight)
          ...
          <header>
          @auth
          @if($overnight->name == auth()->user()->name || 'admin')
          <div class="text-right">
          <a href="/deletesreview/{{$overnight->id}}" class="w-28 text-sm bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-1 rounded-lg">delete</a>
          </div>
          @endif
          @endauth
            <a href="#" class="text-black no-underline">
              <span class="font-medium">{{$overnight->name}}</span>
              <span class="font-normal text-grey">{{$overnight->email}}</span>
            </a>
            <div class="text-xs text-grey flex items-center my-1">
              ...
              <span>{{$overnight->created_at->diffForHumans()}} </span>
            </div>
          </header>
          ...
          @if($overnight->likes_count > 1)
          {{$overnight->likes_count}} likes
          @else
          {{$overnight->likes_count}} like
          @endif
          </p>
          @auth
          <footer class="border-t border-grey-lighter text-sm flex">
          <form method="POST" action="/comments/{{$overnight->id}}/like">
          @csrf
            <button type="submit" class="block no-underline {{$overnight->isLikedBy(auth()->user()) ? 'text-blue-600' : 'text-blue'}} flex px-4 py-2 items-center hover:bg-grey-lighter">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
